...
if [ -d /root/.local/share/eosio/nodeos/data ] && [ $fresh_chain -eq 0  ]; then
  exec /usr/local/bin/nodeos "$@"
else
  if [ ! -f /root/.local/share/eosio/nodeos/config/config.ini ]; then 
    mkdir -p /root/.local/share/eosio/nodeos/config
    wget -O /root/.local/share/eosio/nodeos/config/config.ini https://raw.githubusercontent.com/UOSnetwork/uos.docs/master/testnetv1/config.ini
  fi
  ...
  exec /usr/local/bin/nodeos --genesis-json /root/.local/share/eosio/nodeos/config/genesis.json "$@"
fi
